...
\tutor{Te lo pregunto de otra forma, /create-route es un endpoint HTTP o WebSocket? Si es lo primero, debería ir con los endpoints, si es lo segundo, borra estos comentarios y no hables de rutas HTTP en la subseccioón "Eventos disponibles", habla de endpoints WebSocket. Si es necesario, renombra también la subsección a "Eventos WebSocket" o "Endpoints WebSocket".}
...
Además de las rutas HTTP, el servidor permite la comunicación WebSocket utilizando los siguientes eventos:
...
\tutor{OJO!!! El siguiente docker-compose.yml solo sirve para desplegar en Okteto, cambia las variables (y comprueba que funciona) para lanzarlo en local}
...
\tutor{Asume que saben Git y plantea solo esa opción, ya que queda muy verboso}
